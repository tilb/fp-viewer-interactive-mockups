...
//VAL-5135 - Dummy Console object
//Code taken from http://code.google.com/p/fbug/source/browse/branches/firebug1.5/lite/firebugx.js
if (!window.console) {
...
function WaardeVan_Change(Van, Tot) {
    ...
    if (SelectVan.options[SelectVan.selectedIndex].value == SelectTot.options[SelectTot.selectedIndex].value) {
        ...
        SelectTot.selectedIndex = n;
    } else if (parseInt(SelectVan.options[SelectVan.selectedIndex].value) > parseInt(SelectTot.options[SelectTot.selectedIndex].value)) {
...
function WaardeTot_Change(Van, Tot) {
	...
	if (SelectVan.selectedIndex === 0 || SelectTot.selectedIndex === 0) {
	    //Voor anders geen checkboxes aanpassen
	    return;
	}
...
function read_realtor_alt_email() {
    var realtorUid = readCookie('FD_Gelukt_UserName');
    if (realtorUid != null && realtorUid != '') {
        var cookieVal = readCookie(realtorUid + '_alt_email');
...
function set_realtor_alt_email(altEmail) {
    var realtorUid = readCookie('FD_Gelukt_UserName');
    if (realtorUid != null && realtorUid != '') {
        createCookie(realtorUid + '_alt_email', altEmail);
    }
}

function erase_realtor_alt_email() {
    var realtorUid = readCookie('FD_Gelukt_UserName');
    if (realtorUid != null && realtorUid != '') {
        eraseCookie(realtorUid + '_alt_email');
    }
}
        ...
        return function () {
	        ...
	        $(saveDivSelector).each(function () {
	            if ($(this).attr("id") == "pg-option-save-btn") {
	                $(this).html('<span class="icn-favor"></span>' + FTranslate.Get("TabBewaard"));
...
function object_clientactie(soortactie, url, succesUrl, saveDivSelector, followDivSelector, callFunctieNaComplete, blocking) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != '') {
                if (!data.bewaard) {
                    var meerdanmelding = 'U kunt geen woningen meer bewaren, want u heeft het maximum van ' + data.maxAantal + ' overschreden.\r\nU kunt bewaarde woningen verwijderen in Mijn funda.';
                    bewaard = false;
                    ...
                    //$(this).addClass('button-l');

                    alert(meerdanmelding);
...
// submenu
var Funda = Funda || {};
Funda.subMenu = function () {

    // We use focus and blur events for keyboard accessibility
    // and tabindex="0" so the arrow is tabbable

    var toggleOpen = function (elem) {
        $(elem).parents('li.has-subnav').toggleClass('down');
    };

    var _init = function () {

        $('#nav-global li.has-subnav > a').append('<span tabindex="0">Toon type objecten</span>');

        var nextElement = $('#nav-global li.has-subnav').next().find('a');

        $(document)
            .on('focus', '#nav-global li.has-subnav a span', function (e) {
                toggleOpen(this);
            })
            .on('click', '#nav-global li.has-subnav a span', function (e) {
                e.preventDefault();
            })

            // click anywhere other than the dropdown closes the dropdown
            .on('click', function (e) {
                var $target = $(e.target),
                    isNotSubMenu = !($target.hasClass('sub-nav') || $target.parents().hasClass('has-subnav'));

                // if visible submenu and click not on submenu
                if ($('.sub-nav:visible').length && isNotSubMenu) {
                    $('li.has-subnav').toggleClass('down');
                }
            });
            nextElement.on('focus', function (e) {
                $('li.has-subnav.on').removeClass('down');
            });
...
function getUrlWithoutHttps() {
    var virtualDir = '';
    var replacedUrl = self.location.href.toLowerCase().replace("https://","http://");
    if (replacedUrl.indexOf('/fundadesk/') > 0) virtualDir = replacedUrl.substring(0, replacedUrl.indexOf('/fundadesk/')) + '/fundadesk';
    else if (replacedUrl.indexOf('/funda/') > 0) virtualDir = replacedUrl.substring(0, replacedUrl.indexOf('/funda/')) + '/funda';
    else if (replacedUrl.indexOf('/fundainbusiness/') > 0) virtualDir = replacedUrl.substring(0, replacedUrl.indexOf('/fundainbusiness/')) + '/fundainbusiness';
    else if (replacedUrl.indexOf('www.funda.nl') > 0) virtualDir = replacedUrl.substring(0, replacedUrl.indexOf('www.funda.nl')) + 'www.funda.nl';
    else if (replacedUrl.indexOf('www.fundadesk.nl') > 0) virtualDir = replacedUrl.substring(0, replacedUrl.indexOf('www.fundadesk.nl')) + 'www.fundadesk.nl';
    else if (replacedUrl.indexOf('www.fundainbusiness.nl') > 0) virtualDir = replacedUrl.substring(0, replacedUrl.indexOf('www.fundainbusiness.nl')) + 'www.fundainbusiness.nl';
    return virtualDir;
}
...
* @classDescription Adds behavior to object connection (seller/buyer) frontend
* @return {Object} init
*/
...
Connection = function () {
    ...
    */
        ...
        */
        init: function () {
            $('*[placeholder]').each(function () {
                // make sure the field does not already have focus
                if (($(this).val() === '' || $(this).val() === $(this).attr('placeholder')) && $(this).is(":focus") == false) {
    ...
    Form.selectRow = {
        init: function () {
            ...
            $('div.connection-list ul li').each(function () {
                var row = $(this),
			input = $(this).find('input:radio');
                if (input.is(':checked')) row.addClass('selected');

                input.change(function () {
                    row.parent('ul').children('li').removeClass('selected');
    ...
    */
    var _init = function () {
        // create Sliders
        $('div.property-connections').each(function () {
            if ($(this).find('li.connection').length > 1) {
                var list = $(this).find('ul:not(.link-list)');
                var slider = new Slider(list);
            }
        });
        ...
        // handle placeholder attr in form input fields, but make sure that the browser does not support it first
        if (('placeholder' in document.createElement('input')) == false) {
            Form.placeHolders.init();
        }
        Form.selectRow.init();
    };
...
//na aanleiding mail jeroen mulder ma-8-8-2011
$(document).ready(function () {
    Funda.subMenu.init();
});


function resizeSpanAfterLoad(i, timeoutCounter, specsTable, arrayOfAds, myFrame, widthChecked) {
    if (myFrame.length == 0 && timeoutCounter < 10) {
        setTimeout(function () { resizeSpanAfterLoad(i, ++timeoutCounter, specsTable, arrayOfAds, myFrame, widthChecked); }, 150);
    } else {
        var myElement = $(myFrame[0]).contents().find("span#setWidth");
        ...
        if (myElement.length == 0) {
            ...
            myElement.css("width", widthChecked + "px");
        }
    }
};

function setWidthOfAds(timeoutCounter){
	  var specsTable = $(".specs-cats");
		var arrayOfAds = specsTable.find(".specs-cats-ad");
		var arrayOfTH = specsTable.find('th');
		var widthChecked = arrayOfTH[1].offsetWidth-5;
		for (i = 0; i < arrayOfAds.length; i++) {
				var myFrame = arrayOfAds[i].getElementsByTagName('iframe');
				resizeSpanAfterLoad(i, 1, specsTable, arrayOfAds, myFrame, widthChecked);
		}
};

// tiny Pub/Sub (ben alman)
(function ($) {
    'use strict';
    var o = $({});
...
;
﻿var loaderHtml = '<div id="popup-content-range" class="popup-content popup-content-range">\
    <div class="wbb">\
        <a class="close-popup popup-close" title="Sluiten" href="#"><img alt="sluiten" src="/img/kaart/marker/map-icn-close.gif"></a>\
        <h2 class="hdr">&nbsp;</h2>\
        <fieldset class="loading" style="height: 100px;">\
        </fieldset>\
    </div>\
</div>';

function CheckNofollowRel(rel) {
    if (rel !== null && rel.indexOf(Popup.classNames.nofollow) !== -1) {
        rel = rel.replace(Popup.classNames.nofollow, "");
        rel = $.trim(rel);
    }
...
var Popup =
{
            ...
            if (!$obj.hasClass(Popup.classNames.done)) {
                ...
                if (rel) {
                    ...
                    if (isAjaxContent) {
                        ...
                        //Gebruik ajaxloader bij klikken link... (met uitzondering van save-button op resultlist)
                        var useLoader = !($obj.hasClass('save') || $obj.hasClass('popup-noloader'));
                        ...
                        $obj.click(function (originalOnClick, actionTexts, useLoader, rel) {
                            return function (e) {
                                if (useLoader) {
                                    Popup.fill(loaderHtml);
                                }

                                Popup.load.call(this, originalOnClick, actionTexts);
                                e.preventDefault();
                            };
                        }(originalOnClick, actionTexts, useLoader, rel));
                    }
                    else {
    ...
    load: function (originalOnClick, actionTexts, rel) {
        ...
        if (Popup.popupLoginSuccessful && rel === 'popup-content-login') {
            ...
            thisElement = this.id ? $('#' + this.id) : $('a[rel="' + rel + '"]');
            Popup.showAjaxView(rel, thisElement.data('popup'), thisElement.attr('popupclass'), thisElement.attr('controller'), thisElement.attr('action'));
        }
    },

    showAjaxView: function (viewName, data, altClass, controller, action) {
        controller = controller || "lightbox";
        action = action || "getview";

        //some browsers interpret the string after # as a region and don't use the proper url in the ajax call, so we need to explicitely encode it
        viewName = viewName.replace(/#/g, '%23');

        // opvragen view + posten gelinkte data
        $.ajax(
		{
		    async: true,
		    url: "/" + controller + "/" + action + "/?view=" + viewName + "&ran=" + Math.random(),
		    ...
		    success: function (viewHtml) {
		        ...
		        //fill html
		        var popupClass = (typeof (altClass) === 'undefined' ? viewName : altClass);
		        Popup.fill(viewHtml, popupClass);
		    },
		    type: 'POST'
		});
    },

    fill: function (viewHtml, popupClass) {
        //set content
        ...
        // reset css classes for topmost div and add current viewName as class
        $('#overlay-container').removeClass().addClass('overlay-container').addClass('hide').addClass(popupClass);

        $(Popup.overlayContainer).find(".show-after-load").show();

        Popup.open(Popup.overlayContainer);
        
        Popup.focusOnTextInput();
    ...
    open: function (ele) {
        ...
        Popup.positioningContainer = ele;
        $ele = $(ele);
    ...
    align: function () {
        var ele = Popup.positioningContainer;
    ...
    handleHtmlReplacements: function (htmlReplacements) {
        if (htmlReplacements) {
            ...
            // Make mijnFunda menu work again
            Funda.dropDown.init();
        }
    },
    ...
    focusOnTextInput: function () {
        var textinputs = $(Popup.overlayContainer).find(':input:text:enabled:first');

        if (textinputs.length == 0)
            textinputs = $(Popup.overlayContainer).find("textarea:first");

        if (textinputs.length > 0) {

            var firstTextInput = textinputs[0];

            firstTextInput.onfocus = function () {
                Popup.moveCaretToEnd(firstTextInput);
...
﻿$(document).ready(function () {
    ...
    if (document.referrer.match('/bladeren/')) {
        if (getQuerystring(document.referrer, 'meer') != "") {
            source = "bladeren - meer";
        } else {
            source = "bladeren";
        }
    } else if (document.referrer.match('/postcode/')) {
        source = "uitgebreid zoeken - postcode";
    } else if (document.referrer.match('/adres/')) {
        source = "specifiek adres";
    } else if (document.referrer.match('/uitgebreid-zoeken/')) {
        source = "uitgebreid zoeken - plaats";
    } else if (document.referrer.match('/makelaars/')) {
        source = "Makelaars aanbod";
        if (document.location.pathname.match('/makelaars/.*/koop/heel-nederland/.+')) {
            trackEvent = false;
        } else if (document.location.pathname.match('/makelaars/.*/koop/heel-nederland/')) {
            trackEvent = true;
        } else if (document.location.pathname.match('/makelaars/')) {
            trackEvent = checkPriceChanged();
            if (!trackEvent) {
                trackEvent = checkAreaChanged();
            }
        }
    } else if (document.referrer.match('/galerij/')) {
        source = "galerij - filters";

        if (document.location.pathname.match('/galerij/')) {
            trackEvent = checkPriceChanged();
            if (!trackEvent) {
                trackEvent = checkAreaChanged();
            }
        }
    } else if (document.referrer.match('/kaart/')) {
        source = "kaart - filters";

        if (document.location.pathname.match('/kaart/')) {
            trackEvent = checkPriceChanged();
            if (!trackEvent) {
                trackEvent = checkAreaChanged();
            }
        }
    } else if (document.referrer.match('/(nieuwbouw|koop|huur|recreatie|europe|kantoor|bedrijfshal|winkel|horeca|bouwgrond|garagebox|paktijkruimte|showroom|overig)/.+')) {
        source = "resultaatlijst - filters";
    ...
    } else if (document.referrer.match('funda')) {
        if (document.location.pathname.match('galerij')) {
            source = "Woning in beeld";
        } else {
            source = "homepage";
        }
    } else {
    ...
    if (trackEvent) {
        _gaq.push(['_trackEvent', 'Zoekgedrag', source, document.location.pathname, 0]);
    }

    $("#iseaq a").live("click", function () {
        var domEl = $(this).get(0);

        if (domEl.href.match('http.*')) {
            var source = "";
            if (getPathName(domEl.href).match('/galerij/')) {
                source = "galerij - iSEAQ";
            } else if (getPathName(domEl.href).match('/kaart/')) {
                source = "kaart - iSEAQ";
            } else if (getPathName(domEl.href).match('/makelaars/')) {
                source = "makelaars - iSEAQ";
            } else {
                source = "resultaatlijst - iSEAQ";
            }

            _gaq.push(['_trackEvent', 'Zoekgedrag', source, getPathName(domEl.href), 0]);
        }
    });

    $("#related-ftr a").live("click", function () {
        var domEl = $(this).get(0);

        if (domEl.href.match('http.*')) {
            _gaq.push(['_trackEvent', 'Zoekgedrag', 'resultaatlijst - SEO blok', getPathName(domEl.href), 0]);
        }
    });

    $("#filter a, .filter a").live("click", function () {
        var domEl = $(this).get(0);
        ...
        if (domEl.href.match('http.*')) {
            ...
            if (getPathName(domEl.href).match('/galerij/')) {
                ...
                source = "makelaars - filters";
            }
            else {
                source = "resultaatlijst - filters";
            }

            if (getQuerystring(domEl.href, 'meer') == "") {
                _gaq.push(['_trackEvent', 'Zoekgedrag', source, getPathName(domEl.href), 0]);
            }
        }
    });
    ...
    function checkPriceChanged() {
        var destUrl = document.location.pathname.match('/[0-9]+(\\+|-[0-9]+)/')
        var sourceUrl = document.referrer.match('/[0-9]+(\\+|-[0-9]+)/')

        //if there is price range. in the source or destionation URL (if AND else if)
        //else we pressed a link on the page and not the prijs aanpassen button. So we do not need to track an event.
        if (destUrl !== null && destUrl.length > 0) {
            if (sourceUrl !== null && sourceUrl.length > 0) {
                //if source and destination prices are different track an event
                //if we have a destination price but no source price track an event
                if (destUrl[0] !== sourceUrl[0]) {
                    return true;
                } else {
                    return false;
                }
    ...
    function checkAreaChanged() {
        var destUrl = document.location.pathname.match('/[0-9]+(\\+|-[0-9]+)-opp/')
        var sourceUrl = document.referrer.match('/[0-9]+(\\+|-[0-9]+)-opp/')
    ...
    function getPathName(urlToStrip) {
        return '/' + /https?:\/\/.*?\/(funda\/|fundainbusiness\/)?(.*)/.exec(urlToStrip)[2];
    }

    function getQuerystring(urlToSearch, key, default_) {
        if (default_ == null) default_ = "";
        key = key.replace(/[\[]/, "\\\[").replace(/[\]]/, "\\\]");
        var regex = new RegExp("[\\?&]" + key + "=([^&#]*)");
        var qs = regex.exec(urlToSearch);
        if (qs == null)
            return default_;
        else
            return qs[1];
...
/*global Funda*/
   ...
   // Overview: <br/>
   // Flexible module for creating dropdowns.
   // Dropdown returns the dropdown factory,
   // adds itself as a jQuery plugin and
   // initializes all dropdowns across the sites.
   ...
   ***********************************************************************************
   ...
   ----------------
    <div class="$wrapper">
       <a href="#"  class="$trigger">Text</a>
       <ul class="$dropdown_list">
          <li class="selected"><a href="#">Datum (andersom)</a></li>
          <li><a href="#">Plaats</a></li>
          <li><a href="#">Vraagprijs</a></li>
          <li class="clear"><a href="#">Verwijderen</a></li>
       </ul>
    </div>

   Notes
   ------
   See toolkit for alternate existing structures.


   Init
   ---------
   * automatically initialized
   * manually, with a wrapper class, trigger class and a dropdown-list


   LEGACY SUPPORT
   -------------------
   * All exisiting dropboxes have been incorporated in the start function.
   * Existing classes signifying open elements have been incorporated.
...
var DropDown = (function ($, window, document, undefined) {
    ...
    DropDown.prototype = {
        defaults: {
            wrapper: '.dropdown',           // the parent element
            trigger: '.dropdown-trigger',   // the link that triggers open
            list: '.dropdown-list',         // the dropdown list that shows/hides
            isOpen: '.is-open'              // the class that signifies dropdown isOpen
        },
        ...
        init: function () {
            ...
            // Strip the leading '.' from the value because isOpen is used with
            // hasClass(), toggleClass(), removeClass()
            // which don't accept the '.' when being called
            this.config.isOpen = this.config.isOpen.replace(/^[\.|\#]/, '');

            if (!this.$elem.length) { return false; }

            // Find and cache DOM elements
            this.$trigger = this.$elem.parents(this.config.wrapper).find(this.config.trigger);
            this.$wrapper = this.$trigger.parents(this.config.wrapper);
            this.$list = this.$wrapper.find(this.config.list);

            // Bind Event Handlers
            this.bindHandlers();

            // Return this specific instance
            // return this;

        },

        // Bind event handlers to object and document
        bindHandlers: function () {
            var self = this,
                ...
                t = trigger.replace(/^[\.|\#]/, ''),
                w = wrapper.replace(/^[\.|\#]/, ''),
                lastListItem = this.$list.find('a').get(-1);

            // Bind a click handler on dropdown link
            this.$trigger
                .off("click.dropdowns keydown.dropdowns") // unbind in case init is called twice
                .on("click.dropdowns", function (e) {
                    e.preventDefault();
                    self.onClick(this);
                })
                .on('keydown.dropdowns', function (e) {
                    // open on tabover
                    if ((e.which || e.keyCode) === 9) {
                        // pass the triggering link
                        self.onClick(this);
                    }
                });

            // accessibility for tab navigation
            $(lastListItem).on('blur.dropdowns', function (e) {
                self.onClick(self.$trigger);
            });


            // Add document events
            // Bind keypress event handler (cursor navigation)
            $(document)
                .on({
                    "keydown.dropdowns": function (e) {
                        self.cursorOption(e);
                    },

                // Hide dropdown on click somewhere on the page
                    "click.dropdowns": function (e) {
                        // Boolean: true if any dropdown is open on page
                        var isAnyOpen = $(e.target).parents('a').hasClass(t) || $(e.target).parents('div').hasClass(w);
        ...
        onClick: function (trigger) {
            ...
            isActive = $trigger.hasClass(self.config.isOpen); // true if open

            self.closeOpenDropdowns();

            if (!isActive) {
                $trigger.toggleClass(self.config.isOpen);
                this.$list.show();
            }
        },
        closeOpenDropdowns: function () {

            var $trigger =  $(this.config.trigger + '.' + this.config.isOpen),
                $list;

            // Two checks ensures binding execute on the proper element
            // (in case of multiple dropdowns structures sharing the same trigger name)

            // Check that the element exists
            if (!$trigger.length) { return false; }
            $list = $trigger.parents(this.config.wrapper).find(this.config.list);

            // Check that it has a matching list
            if (!$list.length) { return false; }
            $trigger.removeClass(this.config.isOpen);
            $list.hide();
        },
        cursorOption: function (e) {
            var self = this,
                keyCode = e.keyCode || e.which,
                key = {left: 37, up: 38, right: 39, down: 40, esc: 27, enter: 13 },
                $items,
                index,
                current;


            // Don't bother going further if keypress wasn't on a key we are interested in
            if (!/(37|38|39|40|27|13)/.test(keyCode)) { return; }

            // or if the list isn't visible
            if (!self.$list.is(':visible')) { return; }

            // Find the items we are interested in
            current = self.$list.find('li.selected');

            // cache items and track selected item via an index
            $items = $(self.$list.find('li'));
            index = $items.index($items.filter('.selected')); // integer

            // ENTER or ESC
            // we don't prevent default and we close all dropdowns
            if (keyCode === key.enter || keyCode === key.esc) { self.closeOpenDropdowns(); }

            // Otherwise prevent default
            if (keyCode !== key.enter) { e.preventDefault(); }

            // RIGHT
            if (keyCode === key.right) {
                current.find(this.config.trigger).click();
                self.closeOpenDropdowns();
            }

            // DOWN

            if (keyCode === key.down && index < $items.length) { index++; }
            if (keyCode === key.down && index === $items.length) { index = 0; }

            // UP
            if (keyCode === key.up && index === 0) { index = $items.length; }
            if (keyCode === key.up && index > 0) { index--; }

            // Only execute on down/up
            if (keyCode === key.down || key.up) {
                current.removeClass('selected');
                $items.eq(index).toggleClass('selected').focus();
            }
    ...
    // jQuery Bridge: allows us to use this has a jquery plugin
...
// # Initialize dropdowns in Funda namespace.
// All variations across the sites are initialized here.
Funda = window.Funda || {};
Funda.dropDown = (function () {
    'use strict';

    var _init = function () {

        // Override all legacy instances of Dropdown.init() in the html
        var Dropdown = {};
        Dropdown.init = function (x) { return false; };
        window.Dropdown = Dropdown;

        // ## The variations.
        // Resultslist Filter
        var dropdownResultsListTrigger = $('.select-list-field'),
            i,
            $this,
            dropdownResultsList;

        // Using a for loop is faster than $.each()
        for (i = dropdownResultsListTrigger.length; i--;) {
            $this = $(dropdownResultsListTrigger[i]);
            dropdownResultsList = new DropDown($this, {
                wrapper: '.dropdown-box',
                trigger: '.select-list-field',
                list: '.select-list',
                isOpen: '.down'
        ...
        // Homepage box-tab-more button
        // conflicts will result if the trigger name in the html is the same while the rest of the classnames are different.
        var dropdownMoreTrigger = $('#more-button');

        dropdownMoreTrigger.Dropdown({
            trigger: '.more',
            list: '#more-dropdown',
            wrapper: '.box-tab-more'
        });

        // My Funda account
        var dropdownMyFundaTrigger = $('.my-menu-button');

        dropdownMyFundaTrigger.Dropdown({
            trigger: '.my-menu-button',
            list: '.my-menu-dropdown',
            wrapper: '.my-menu-container'

        });

        // FD Wijzig knop
        var dropdownWijzigTrigger = $('.action-edit-dropdown > a');

        // Check if it exsists before trying to add a class
        if (dropdownWijzigTrigger) {

            // Add trigger class to support the dropdown module without changing html
            dropdownWijzigTrigger
                .addClass('trigger')
                .Dropdown({
                    trigger: '.trigger',
                    list: '.nav-dropdown',
                    wrapper: '.action-edit-dropdown'
                });
        }
        var dropdownkantoorTrigger = $('.button-dropdown > a');

        // Check if it exsists before trying to add a class
        if (dropdownkantoorTrigger) {

            // Add trigger class to support the dropdown module without changing html
            dropdownkantoorTrigger
                .addClass('trigger')
                .Dropdown({
                    trigger: '.trigger',
                    list: '.nav-dropdown',
        ...
        // FM Resultslist
        var dropdownFMSortTrigger = document.getElementById('sort-button'),
            dropdownFMSort = new DropDown(dropdownFMSortTrigger, {
                trigger: '#sort-button',
                list: '#sort-dropdown',
                wrapper: '.listing-sort-container'
            });
    };
...
﻿var FlashDLBanner = {
    ...
    Script: '<div class="noscript"><p><strong class="txt-lb">Voor <Media> is Adobe Flash Player vereist</strong><br/><a href="http://get.adobe.com/flashplayer" class="ext-site">Download de laatste versie van de Adobe website</a><p></div>',
    HasFlashSupport: false,
    jqsNoScript: null,
    jqsClass: null,
    displayText: null,
    resize: false,
    removeOtherChildren: false,

    init: function (jqsNoScript, jqsClass, displayText, resize, removeOtherChildren) {
        // save references
        FlashDLBanner.jqsNoScript = jqsNoScript;
        FlashDLBanner.jqsClass = jqsClass;
        FlashDLBanner.displayText = displayText;
        FlashDLBanner.resize = resize;
        FlashDLBanner.removeOtherChildren = removeOtherChildren;

        // check if correct flash version can be found
        if ((typeof (deconcept) !== 'undefined' && deconcept.SWFObjectUtil.getPlayerVersion().major > 9)
         || (typeof (swfobject) !== 'undefined' && swfobject.getFlashPlayerVersion().major > 9)) {
            FlashDLBanner.HasFlashSupport = true;
            return;
        }
    },

    activate: function () {
        if (!FlashDLBanner.HasFlashSupport) {
            var curScript = FlashDLBanner.Script.replace('<Media>', FlashDLBanner.displayText);
            // does script already exist in this context
            if ($('div.noscript', FlashDLBanner.jqsNoScript).length == 0) {
                $(FlashDLBanner.jqsNoScript).empty().append(curScript);
            }
            if (!$(FlashDLBanner.jqsClass).hasClass('upgrade-flash-plugin')) {
                $(FlashDLBanner.jqsClass).addClass('upgrade-flash-plugin');
            }
            if (FlashDLBanner.resize) {
                $('div.noscript', FlashDLBanner.jqsNoScript).attr('style', 'height:100%');
            }
            if (FlashDLBanner.removeOtherChildren) {
                $(FlashDLBanner.jqsNoScript).children(':not(div.noscript)').remove();
            }
            // exception for some missing css
            if (FlashDLBanner.jqsNoScript == 'div#cb_flash' && FlashDLBanner.jqsClass == 'div.map-sv') {
                $('div.noscript', FlashDLBanner.jqsNoScript).attr('style', $('div.noscript', FlashDLBanner.jqsNoScript).attr('style') + ';display:block !important;');
        }
            if (!$('div.noscript', FlashDLBanner.jqsNoScript).parents().hasClass('streetview-container')
             && !$('div.noscript', FlashDLBanner.jqsNoScript).parents().hasClass('map-sv-container')
             && !$('div.noscript', FlashDLBanner.jqsNoScript).parents().hasClass('blueprint-app')
             && !$('div.noscript', FlashDLBanner.jqsNoScript).parents().hasClass('app-container')) {

                $('div.noscript', FlashDLBanner.jqsNoScript).parent().addClass('streetview-container');
    }
        }
    }
};
...
// --------------------
// Added functionality specifically for validating signup forms.
// Signup forms contain an email, password and password check field, together with an error and help element in their input group.
// --------------------

ValidateSignUpForm = function (emailInput, passwordInput, passwordCheckInput, checkForFieldsAreEmpty) {

    var errorEmail = emailInput.parents("p.input-group").children(".input-error-desc");
    var errorPassword = passwordInput.parents("p.input-group").children(".input-error-desc");
    var errorPasswordCheck = passwordCheckInput.parents("p.input-group").children(".input-error-desc");

    // Cleanup previous set error messages.
    errorEmail.empty().hide();
    errorPassword.empty().hide();
    errorPasswordCheck.empty().hide();

    //Email field has no help text
    var helpPassword = passwordInput.parents("p.input-group").children(".input-expl");
    var helpPasswordCheck = passwordCheckInput.parents("p.input-group").children(".input-expl");

    //Display help text
    helpPassword.show();
    helpPasswordCheck.show();

    //Error messages are allowed to accumulate, so we return the validness of the form in the end.
    var formIsValid = true;

    // Validate email correctness                    
    if (!checkRequiredVisibleField_MessageBehindField(emailInput, errorEmail,
        FTranslate.Get('LbEmailAdres'),
        function (value) { return checkEmail(value); },
        FTranslate.Get('Vuleenemailadresin'),
        FTranslate.Get('errorNietCorrectAchterVeldValidatie'),
         checkForFieldsAreEmpty)) {
        formIsValid = false;
    }

    // validate password minimum length
    if (!checkRequiredVisibleField_MessageBehindField(passwordInput, errorPassword,
            FTranslate.Get('txtWachtwoord'),
            function (value) { return checkPasswordLengthNotToSmall(value); },
            FTranslate.Get('Kieseenwachtwoord'),
            FTranslate.Get('Kieseenwachtwoordvan6tekens'),
             checkForFieldsAreEmpty)) {
        // hide help text to make room for error text. 
        helpPassword.hide();
        formIsValid = false;
    }

    // validate password is equal to password check
    if (!checkRequiredVisibleField_MessageBehindField(passwordCheckInput, errorPasswordCheck,
        FTranslate.Get('txtWachtwoord'),
        function (value) { return value == passwordInput.val(); },
        FTranslate.Get('Herhaalhetwachtwoord'),
        FTranslate.Get('Komtnietovereenmetwachtwoord'),
         checkForFieldsAreEmpty)) {
        //Hide helptext
        helpPasswordCheck.hide();
        formIsValid = false;
    }
...
/* matches numbers with a fractional part (dutch locale; thousand seperators are not allowed)
   following is allowed:
      112 (whole numbers)
      ,75 (only fraction)
      1,5 ('complete' number with fraction)
   the following is not:
      , (only a comma)
      4, (number with comma; without fractional part)    */
...
var errorNoRadioButton = "Er is geen keuze gemaakt bij <strong>{0}</strong>. Selecteer een van de opties.";

function checkRadioButtonsChecked(radiobuttonList, notifyWrapper, veldNaam) {
    if (radiobuttonList.length != 1) {
        var veldNietCorrectTekst = errorNoRadioButton.replace('{0}', veldNaam);
        toonFoutmelding(radiobuttonList, notifyWrapper, ((veldNaam != null && notifyWrapper.html() == '') ? veldNietCorrectTekst : FTranslate.Get("errorMeerdereVerplicht")));
...
function checkRequiredVisibleField(inputWrapper, notifyWrapper, veldNaam, extraCheckFunction, extraCheckFoutmelding, alternatieveVerplichtFoutmelding) {
    if (inputWrapper && inputWrapper.is(":visible")) {
        //toon fout indien value leeg is (of gelijk aan placeholder), of de optioneel meegegeven controle-functie false teruggeeft
        if (inputWrapper.val() == "" || inputWrapper.val() == inputWrapper.attr('placeholder')) {
            var veldNietCorrectTekst = alternatieveVerplichtFoutmelding || FTranslate.Get("errorVerplichtWithField").replace('{0}', veldNaam);
